...
export default function Calendar() {
  return (
    <div className="flex flex-col gap-8 py-8 px-4 sm:px-8 md:px-12 w-full max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative">
        {CALENDAR_DATA.map((item, idx) => (
          <div key={idx} className="glass-surface p-6 rounded-3xl transition-transform duration-300 hover:-translate-y-1 hover:shadow-2xl flex flex-col relative overflow-hidden group">
            {/* Background accent line */}
            <div className="absolute top-0 left-0 w-full h-1" style={{ backgroundColor: item.color }} />
            
            <h2 className="text-2xl font-bold uppercase tracking-wider mb-5" style={{ color: item.color, fontFamily: "'Rajdhani','Inter',sans-serif" }}>
              {item.month}
            </h2>
            ...
            <div className="flex-1 space-y-5">
              {item.events.map((event, eIdx) => (
                <div key={eIdx} className="relative pl-4 border-l-2" style={{ borderColor: `${item.color}40` }}>
                  <div className="absolute -left-[5px] top-1.5 w-2 h-2 rounded-full" style={{ backgroundColor: item.color }} />
